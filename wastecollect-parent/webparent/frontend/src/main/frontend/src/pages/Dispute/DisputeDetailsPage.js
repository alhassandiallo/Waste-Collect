...
// --- START OF CONFIRMATION (NO CHANGES EXPECTED HERE) ---
import AuthContext from '../../contexts/AuthContext'; // Correct path from src/pages/dispute to src/contexts
import disputeService from '../../services/disputeService'; // Correct path from src/pages/dispute to src/services
import LoadingSpinner from '../../components/LoadingSpinner'; // Correct path from src/pages/dispute to src/components
// --- END OF CONFIRMATION ---

/**
 * Composant DisputeDetailsPage - Affiche les détails complets d'un litige spécifique
 * Cette page est conçue pour être accédée via un chemin avec un ID de litige (e.g., /disputes/:disputeId)
 *
 * Fonctionnalités:
...
const DisputeDetailsPage = () => {
     ...
     * ou lorsque l'ID du litige change.
     */
    useEffect(() => {
        if (disputeId) { // Only load if disputeId is present
            loadDisputeDetails();
        } else {
            setLoading(false);
            setError("Aucun ID de litige fourni. Veuillez sélectionner un litige pour voir les détails.");
        }
    }, [disputeId]); // Add disputeId to dependency array
    ...
    const loadDisputeDetails = async () => {
        try {
            ...
            // Assuming disputeService.getDisputeDetails returns { data: {...} } or direct object
            const response = await disputeService.getDisputeDetails(disputeId);
            setDispute(response?.data || response); // Handle both potential return structures
        } catch (err) {
            ...
            setDispute(null); // Clear dispute data on error
        } finally {
            setLoading(false);
        }
    };
    ...
    const getStatusBadgeVariant = (status) => {
        switch (status?.toLowerCase()) {
            case 'nouveau':
            case 'new':
            case 'pending': // Ensure this matches backend enum if applicable
                return 'primary';
            case 'en_cours':
            case 'in_progress':
                return 'warning';
            ...
            case 'ferme':
            case 'closed':
            case 'rejected': // Add rejected to a similar color or distinct one
                return 'secondary';
            case 'rejete': // Add this for clarity
                return 'danger';
            case 'escalated': // New status from DisputeList
                return 'info';
            default:
                return 'secondary';
        }
    ...
    const getPriorityBadgeVariant = (priority) => {
        switch (priority?.toLowerCase()) {
            ...
            case 'basse':
            case 'low':
            case 'normal': // New status from DisputeList
                return 'info';
            default:
                return 'secondary';
        }
    ...
    const handleStatusChange = async () => {
        ...
        try {
            ...
            const updateData = {
                ...
                updatedBy: user.id // Assuming user.id is available
            };

            // Appel au service pour mettre à jour le statut
            await disputeService.updateDisputeStatus(disputeId, newStatus, statusChangeReason, user.id); // Adjusted to match disputeService.updateDisputeStatus signature
    ...
    const handleAddComment = async () => {
        ...
        try {
            ...
            const commentData = {
                content: newComment,
                authorId: user.id, // Assuming user.id is available
                authorName: user.firstName ? `${user.firstName} ${user.lastName}` : user.name, // Handle potential user.name fallback
                disputeId: disputeId // Ensure this matches what backend expects
            };

            // Appel au service pour ajouter le commentaire
            await disputeService.addComment(disputeId, commentData); // Adjusted to match disputeService.addComment signature
            ...
            setError(err.message || 'Impossible d\'ajouter le commentaire. Veuillez réessayer.');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleDownloadAttachment = async (attachmentId, filename) => {
        try {
            ...
            const url = window.URL.createObjectURL(new Blob([response])); // response is likely a Blob from the service
    ...
    const canChangeStatus = () => {
        // Only Admin or Collector can change status
        ...
        return (
            <Container className="mt-4">
                <Alert variant="danger">
                    {error || "Litige introuvable. Veuillez vérifier l'ID du litige."}
                </Alert>
                    ...
                    <Card className="mb-4">
                        ...
                        <Card.Body>
                            <div className="d-grid gap-2">
                                {canChangeStatus() && dispute.status !== 'RESOLVED' && ( // Use 'RESOLVED' to match backend enum
                                    <Button
                                        variant="success"
                                        size="sm"
                                        onClick={() => {
                                            setNewStatus('RESOLVED'); // Use 'RESOLVED'
                                ...
                                {canChangeStatus() && dispute.status !== 'CLOSED' && ( // Use 'CLOSED' to match backend enum
                                    <Button
                                        variant="secondary"
                                        size="sm"
                                        onClick={() => {
                                            setNewStatus('CLOSED'); // Use 'CLOSED'
                                            setShowStatusModal(true);
                                        }}
                                    >
                    ...
                    <Form>
                        <Form.Group className="mb-3">
                            ...
                            >
                                ...
                                <option value="PENDING">En attente</option> {/* Adjusted to match backend */}
